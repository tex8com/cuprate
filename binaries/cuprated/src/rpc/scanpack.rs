...
impl ScanPack {
    pub fn new(
        ...
        if blocks.is_empty()
            || blocks.len() != output_indices.len()
            || blocks.len() > MAX_PACK_BLOCKS
        {
    ...
    pub fn slice_from(&self, start_height: u64, max_blocks: usize) -> Option<Self> {
        ...
        Self::new(
            start_height,
            self.blocks[offset..end].to_vec(),
            self.output_indices[offset..end].to_vec(),
        )
        .ok()
    }
}
...
impl ScanPackStore {
    ...
    pub fn refresh(&self) -> Result<()> {
        ...
        for entry in fs::read_dir(&self.directory)? {
            ...
            let Some(height) = name
                .strip_prefix("pack-")
                .and_then(|v| v.strip_suffix(".mwsp"))
            else {
                continue;
            };
    ...
    pub fn logical_start_height(&self, end: u64) -> u64 {
        if self.max_blocks < 0 {
            ...
            end.saturating_sub(self.max_blocks as u64)
                .max(self.configured_start_height)
        }
    }
    ...
    pub fn newest_short_tail(&self, chunk_blocks: usize) -> Result<Option<(u64, u64)>> {
        ...
        let Some((start, path)) = entry else {
            return Ok(None);
        };
        let pack = read_pack_metadata(&path)?;
    ...
    pub fn replace_short_tail(&self, pack: &ScanPack, chunk_blocks: usize) -> Result<()> {
        ...
        if read_pack_metadata(&final_path)?.block_count >= chunk_blocks {
            bail!(
                "refusing to replace complete scan pack at height {}",
                pack.start_height
            );
        }
        if let Some(path) = previous {
            ...
            if previous.end_height > pack.start_height {
                bail!(
                    "replacement scan pack at {} overlaps preceding pack ending at {}",
                    pack.start_height,
                    previous.end_height
                );
            }
        ...
        if let Some(next_start) = next_start {
            if pack.end_height > next_start {
                bail!(
                    "replacement scan pack ending at {} overlaps following pack at {}",
                    pack.end_height,
                    next_start
                );
            }
        ...
        let temporary_path = self
            .directory
            .join(format!(".pack-{:020}.tmp", pack.start_height));
    ...
    pub fn write_builder_status(
        &self,
        tip_end: u64,
        covered_until: u64,
        live_tail: Option<(u64, u64)>,
        written_packs: u64,
        result: &str,
    ) -> Result<()> {
        ...
        let updated = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_secs();
        let (tail_start, tail_end) = live_tail
        ...
        writeln!(
            writer,
            "missing_blocks={}",
            tip_end.saturating_sub(covered_until)
        )?;
    ...
    pub fn write(&self, pack: &ScanPack) -> Result<()> {
        ...
        let final_path = self
            .directory
            .join(format!("pack-{:020}.mwsp", pack.start_height));
        let temporary_path = self
            .directory
            .join(format!(".pack-{:020}.tmp", pack.start_height));
        write_pack(&temporary_path, pack)?;
        ...
        self.packs
            .write()
            .expect("scan pack index lock poisoned")
            .insert(pack.start_height, final_path);
        Ok(())
    }
    ...
    pub fn remove_before(&self, height: u64) -> Result<usize> {
        let stale: Vec<_> = self
            ...
            .expect("scan pack index lock poisoned")
            .iter()
            .filter_map(|(start, path)| (*start < height).then(|| (*start, path.clone())))
            .collect();
        let mut removed = 0;
...
fn read_bytes(reader: &mut impl Read) -> Result<Vec<u8>> {
    ...
    if len > MAX_ITEM_BYTES {
        bail!("scan pack item exceeds limit")
    };
...
fn write_pack(path: &Path, pack: &ScanPack) -> Result<()> {
    ...
    write_u64(&mut writer, pack.start_height)?;
    write_u64(&mut writer, pack.end_height)?;
    write_u32(&mut writer, u32::try_from(pack.blocks.len())?)?;
    for (block, indices) in pack.blocks.iter().zip(&pack.output_indices) {
        writer.write_all(&[u8::from(block.pruned)])?;
        write_u64(&mut writer, block.block_weight)?;
        write_bytes(&mut writer, &block.block)?;
        match &block.txs {
            TransactionBlobs::Pruned(txs) => {
                writer.write_all(&[1])?;
                write_u32(&mut writer, u32::try_from(txs.len())?)?;
                for tx in txs {
                    write_bytes(&mut writer, &tx.blob)?;
                    writer.write_all(tx.prunable_hash.as_ref())?;
                }
            }
            TransactionBlobs::Normal(txs) => {
                ...
                write_u32(&mut writer, u32::try_from(txs.len())?)?;
                for tx in txs {
                    write_bytes(&mut writer, tx.as_ref())?;
                }
            }
        ...
        for tx in &indices.indices {
            write_u32(&mut writer, u32::try_from(tx.indices.len())?)?;
...
fn read_pack_metadata_from(reader: &mut impl Read) -> Result<ScanPackMetadata> {
    ...
    if &magic != MAGIC {
        bail!("invalid scan pack magic")
    }
    if read_u32(reader)? != FORMAT_VERSION {
        bail!("unsupported scan pack version")
    }
    let start_height = read_u64(reader)?;
    let end_height = read_u64(reader)?;
    let count = usize::try_from(read_u32(reader)?)?;
    if count == 0 || count > MAX_PACK_BLOCKS {
        bail!("invalid scan pack block count")
    }
    if end_height.checked_sub(start_height) != Some(u64::try_from(count)?) {
        bail!("scan pack height interval mismatch")
    }
    Ok(ScanPackMetadata {
        start_height,
        end_height,
...
fn read_pack(path: &Path) -> Result<ScanPack> {
    ...
    let mut output_indices = Vec::with_capacity(count);
    for _ in 0..count {
        let mut pruned = [0; 1];
        ...
        if tx_index_count > MAX_PACK_BLOCKS * 100_000 {
            bail!("scan pack has too many transaction index lists")
        }
        ...
        for _ in 0..tx_index_count {
            ...
            if index_count > MAX_INDICES_PER_TX {
                bail!("scan pack has too many output indices")
            };
...
mod tests {
    ...
    fn pack(start_height: u64, count: usize) -> ScanPack {
        ScanPack::new(
            ...
            (0..count)
                .map(|_| BlockOutputIndices { indices: vec![] })
                .collect(),
        )
        .unwrap()
    }
    ...
    fn persists_and_slices_pack() {
        ...
        let pack = pack(100, 2);
        store.write(&pack).unwrap();
        let read = store.load_covering(101, 10).unwrap().unwrap();
        assert_eq!(read.start_height, 101);
        assert_eq!(read.end_height, 102);
    ...
    fn writes_readable_builder_status() {
        ...
        let store = store(directory.path().to_path_buf());
        store
            .write_builder_status(105, 105, Some((102, 105)), 1, "complete")
            .unwrap();
